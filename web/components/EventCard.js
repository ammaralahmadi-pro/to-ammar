...
export default function EventCard({ event, onEdit, onDelete }) {
  ...
  return (
    <div className="group flex items-start gap-3 rounded-xl bg-white border border-black/5 shadow-card px-4 py-3.5">
      <div className="w-16 shrink-0 pt-0.5 text-xs font-bold text-black/50 tabular-nums">{time}</div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          <h3 className="font-bold text-[15px] truncate">{event.title}</h3>
          <OwnerBadge event={event} />
        </div>
        {event.location && <p className="mt-1 text-xs text-black/50">📍 {event.location}</p>}
        {event.notes && <p className="mt-1 text-xs text-black/40 line-clamp-2">{event.notes}</p>}
      </div>
      <div className="flex flex-col gap-1 shrink-0">
        <button
          onClick={() => onEdit(event)}
          className="text-sm px-2 py-1 rounded-md hover:bg-glaze active:bg-glaze"
